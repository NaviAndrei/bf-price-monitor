...
## T-25 (#33): 30-day Omnibus reference price + fake-discount detection — closed
New `bf_price_monitor/analytics/deal_stats.py` computes, from each alert's prior 30-day window (current observation excluded), the Omnibus reference price, genuine savings %, robust stats (median, 5% trimmed mean, IQR, MAD, stdev, z-score, days at current price) and a `FAKE_DISCOUNT_SUSPECT` flag (advertised original or observed pre-sale price > 1.25x median, new price not below the 30-day low, and an actual drop); shown in Telegram and recorded as a separate AlertDecision reason, never altering `rule_verdict`. Tests: 275 passed (29 new), ruff/mypy clean; replay over real history flagged 11 of 121 drops. Commit: 0862a8e.
Follow-ups not done: Flanco's published 30-day price not verified against live DOM; "Minim 30 zile (Omnibus)" line still shows the previous-price fallback when the window is empty; some flagged real cases show ~3x jumps on one URL that may be listing-identity drift rather than hikes.
## T-26 (#35): Teams, email and ntfy notification channels — committed, not closed
Microsoft Teams (Workflows webhook, Adaptive Card 1.2), email (SMTP) and ntfy delivery added behind the shared outbox with per-watch routing via `Watch.channels`; success, transient, permanent and exhausted-retry/dead-letter outcomes are tested identically for Telegram, Teams, ntfy and email, and SMTP failure reasons redact every configured address. Tests: 333 passed; ruff check and ruff format --check clean. Commit: 2488356. Refs #35 (not closed).
Limitation: no live Teams card has been confirmed in a channel. HTTP 202 proves only that the workflow accepted the request, not that the card was posted or rendered; no real Teams webhook, ntfy server or SMTP server has been contacted. Not pushed yet.
